## What does this pull request do?

Please include a summary of the change and which issue (if any) is referenced:
- Resolves: #<issue-number> (if applicable)
- Description: <brief description>

---

## Motivation / Context

Why is this change needed? What problem does it solve?
(e.g., bug fix, new feature, performance improvement, refactor)

---

## What has been changed

List out key changes:
- Added/changed file `...`
- Updated logic in `cmd/root.go`
- Added tests for `<component>`

---

## How to verify this change

Include steps to test this:
1. Clone and build: `go build -o scraped .`
2. Run: `./scraped -d 1 https://example.com`
3. Verify the behavior: ...

---

## Notes on compatibility

- Platform(s) tested: (e.g., Linux x86_64, macOS arm64)
- Installation method tested: (e.g., Homebrew, go install, from source)
- Version/commit used: `<commit-hash>`
- Any limitations or blockers: `<description>`

---

## Checklist

- [ ] Code changes compile and tests pass
- [ ] Added/Updated relevant documentation
- [ ] Tested on at least one supported platform
- [ ] Added or updated tests (if applicable)

---

## Additional Context (optional)

Add any screenshots, logs, or references to other issues if applicable.
